# Chapter 06: Hindley-Milner and Me
...
Python is a dynamic language, but that does not mean we avoid types all together. We're still working with strings, numbers, booleans, and so on. It's just that there isn't any language level integration so we hold this information in our heads. Not to worry, since we're using signatures for documentation, we can use comments to serve our purpose.

There are type checking tools available for Python such as [mypy](http://mypy-lang.org/). The aim of this book is to equip one with the tools to write functional code so we'll stick with the standard type system used across FP languages.
...
`str_length` is the same idea as before: we take a `String` and return you a `Number`.

The others might perplex you at first glance. Without fully understanding the details, you could always just view the last type as the return value. So for `findall` you can interpret as: It takes a `Regex` and a `String` and returns you `[String]`. But an interesting thing is going on here that I'd like to take a moment to explain if I may.

For `findall` we are free to group the signature like so:
...
```python
# findall:: Regex -> (String -> [String])
# on_holiday :: String -> [String]
on_holiday = findall(r'(holiday)')
```

Each argument pops one type off the front of the signature. `on_holiday` is `fidnall` that already has a `Regex`.
...
# filter :: (a -> Bool) -> [a] -> [a]
filter = curry(lambda f, xs: [x for x in xs if f(x)])
...
```python
# assert_equal :: (Eq a, Show a) => a -> a -> Assertion
```
